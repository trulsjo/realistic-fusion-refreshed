...
is half a screen pixel at zoom 1 — which is why `models/rf_blender.py`'s `SOCKET_Z` calls the height
error it corrected "0.49 px on the sheet, 0.25 px at the game's own zoom", and why a difference that
survives magnification may still be invisible where it is met.
...
`scripts/probe-socket-height.ps1` shot its player's-eye frame at zoom 2 and described that as the
game's own zoom, while `docs/research/socket-shapes.md` — the note `scripts/probe-socket-shapes.ps1`
writes its findings into — had zoom 1 right. The two socket probes used one word for two scales, and
the frame that existed to answer "is this visible" showed every difference at twice its real size.
The scale is now named by number wherever it is shot.
