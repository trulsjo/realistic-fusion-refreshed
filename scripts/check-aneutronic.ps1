...
.DESCRIPTION
    ...
    WHAT IS BUILT
      ...
      dhe3      An rf-aneutronic-reactor fed rf-d-he3-plasma. It must fuse, ignite and settle at
                ...
                neutronic fluid.
      converter An rf-direct-energy-converter BOLTED to that reactor's energy output, with nothing
                between them and nothing that could be between them -- since #87 the fluid carries a
                connection category of its own and no pipe in the game carries it. It must consume
                the fluid and put power on the network, measured as its own contribution rather than
                the network's total, because the reactor's heating is supplied by an energy interface
                on the same network.
                A SECOND converter is CHAINED behind the first, because a stack is how a player lays
                these and the prototype could not do it until it was probed in game: its fluid box
                ...
                Both are placed UNROTATED. ADR 0031 item 4 declares the machine fifteen wide by five
                tall with energy on its long faces, so a converter under or over a reactor bolts on
                as it stands; it used to be declared five by fifteen and this rig had to turn it.
      ...
      tank      An rf-aneutronic-composite-tank, twice: on a helium-3 line, which is the role
                ADR 0018 item 5 leaves it, and against a live categorised energy feed, which it must
                NOT join and must not hold a unit of.

                THIS ROW USED TO BE TRUE AND MEANINGLESS, and it is rewritten rather than dropped
                quietly (#87). It asserted that the tank buffered the tier's energy fluid -- but it
                filled the tank with insert_fluid on a tank that was plumbed to nothing, and Lua
                insertion ignores connection categories. So it would have gone on passing after the
                capability was gone, which is worse than failing. What replaces it asserts the
                capability the tank still has and the one it has lost, both through plumbing.
...
local HELIUM_3 = "rf-helium-3"
local FEED   = "__PLASMAFEED__"
-- The rigs' shared categorised energy feed (#84). An INSTRUMENT rather than something a player has:
-- ADR 0018 ships no pipe carrying either energy fluid, so this is the only thing that can supply
-- one, and the tank row below is measured against it.
...
---
--- THE BOLT ARITHMETIC, NOT THE PIPE-RUN ONE. ADR 0018's Consequences call the difference a trap
--- and it cost #82's rig a false negative on the question deciding that ADR: a pipe run aligns a
--- connection's TARGET onto the tile the pipe occupies, while a bolt aligns one machine's connection
--- TILE onto the other's target. Align target against target and the two machines sit one tile clear
--- of each other pointing at the same empty ground, which looks exactly like a refused connection.
---
--- Placed once as a probe, asked where the connection sits relative to itself, destroyed, placed
--- again by the difference. check-hc.ps1 and check-containment.ps1 carry the same helper, for the
--- reason #49 established: a remembered offset is a hostage to the next prototype edit. This rig
--- learned it twice over -- the converter was three by five when the section below was written, then
--- five by fifteen, and is fifteen by five now.
...
script.on_init(function()
  ...
  -- ------------------------------------------------------------------ the converter
  --
  -- BOLTED ONTO THE REACTOR'S ENERGY OUTPUT WITH NOTHING BETWEEN THEM, which is the whole claim of
  -- the tier: reactor to electricity, one machine, no steam. Since #87 there is nothing that COULD
  -- be between them either -- the fluid carries a connection category of its own and no pipe in the
  -- game carries it.
  --
  -- UNROTATED, WHICH IS WHAT ADR 0031 ITEM 4 BOUGHT. The machine is declared fifteen wide by five
  -- tall with energy on its long faces, so a converter standing north of a reactor meets that
  -- reactor's north output with its own south face as it stands. It used to be declared five by
  -- fifteen with energy east and west, and this rig had to turn it a quarter anticlockwise and
  -- compute the offset from the footprint to make any face meet the reactor at all.
  --
  -- North rather than south because the collector is bolted to the south face below.
  --
  -- Every offset comes off the prototypes through bolt(), because they were written down once and
  -- the machine outgrew them three times: three by five when this rig was built, then five by
  -- fifteen, then fifteen by five.
  local out_north = connection_facing(dhe3, ANEUTRONIC_ENERGY, "north")
  if not out_north then error(ANEUTRONIC .. " has no north-facing energy output") end
  local converter = bolt(surface, force, CONVERTER, ANEUTRONIC_ENERGY, "south",
    out_north.target_position, { 0.5, -30.5 })
  -- No build event raised, unlike the reactors: scripts/entity-management.lua registers reactors
  -- only, so control.lua has nothing to do with a converter and raising one would say it did.

  -- A SECOND CONVERTER CHAINED BEHIND THE FIRST, which is how a player lays a stack of them because
  -- it is how steam turbines have always been laid. It exists because the first version of this
  -- prototype could not do it: the fluid box declared flow_direction = "input" where vanilla's steam
  -- turbine declares "input-output", so two converters back to back did not connect at all and every
  -- one after the first sat dry with nothing on screen to say why. Found on review by probing it in
  -- game, which is the only way it could have been found.
  --
  -- Its south face lands on the first one's north target, so the stack grows AWAY from the reactor.
  -- That is the converter's own chaining axis and not the exchanger's: an exchanger spends both long
  -- faces on energy in and steam out and chains through its short ends instead (ADR 0031 item 5).
  local first_north = connection_facing(converter, ANEUTRONIC_ENERGY, "north")
  if not first_north then error(CONVERTER .. " has no north-facing energy connection to chain through") end
  local second_converter = bolt(surface, force, CONVERTER, ANEUTRONIC_ENERGY, "south",
    first_north.target_position, { 0.5, -30.5 })
  -- Its own supply area, east of the stack rather than in it: the converters now span x -7.5 to 7.5,
  -- so a substation on the centre line would stand inside one. create_entity collision-checks
  -- nothing, so that would build and neither machine would say why it looked wrong.
  power(surface, force, { 12, -12 }, 3e8)
  ...
  -- ------------------------------------------------------------------ the tank, both ways
  --
  -- THE ROW THIS REPLACES WAS TRUE AND MEANINGLESS (#87, ADR 0018's Consequences). It stood one tank
  -- here, called insert_fluid on it with the tier's energy fluid, and asserted that the tank held
  -- some -- but the tank was plumbed to nothing and Lua insertion ignores connection categories, so
  -- the row would have gone on passing after ADR 0018 took the capability away. A gate that cannot
  -- fail is worse than one that does.
  --
  -- So the tank is built twice, and both rows go through PLUMBING:
  --
  --   helium-3   its actual role since ADR 0018 item 5, on an ordinary infinity pipe, because
  --              helium-3 is an ordinary fluid and a player tanks it with ordinary pipes.
  --   energy     against the rigs' own categorised energy feed -- the only thing in the game that
  --              can supply the fluid at all -- which it must not join and must not hold.
  --
  -- The helium-3 row is the control: without it "the tank holds no energy" is a statement about
  -- where this rig put the tank.
  local tanks = {}
  for i, supply in ipairs({ { "infinity-pipe", HELIUM_3 }, { ENERGY_FEED, ANEUTRONIC_ENERGY } }) do
    local tank = must(surface.create_entity({
      name = "rf-aneutronic-composite-tank", position = { 130.5, 0.5 + (i - 1) * 12 }, force = force,
    }), "rf-aneutronic-composite-tank for the " .. supply[2] .. " row")
    -- The supply goes on the tile the TANK's own connection points at -- pipe-run alignment, read
    -- off the entity rather than written down.
    local conn = tank.fluidbox.get_pipe_connections(1)[1]
    local pipe = must(surface.create_entity({
      name = supply[1], position = conn.target_position, force = force,
    }), supply[2] .. " supply for the composite tank")
    pipe.set_infinity_pipe_filter({ name = supply[2], percentage = 1, mode = "at-least" })
    tanks[#tanks + 1] = { tank = tank, supply = pipe, fluid = supply[2] }
  end
  local tank = tanks[1].tank

  storage.rig = {
    dhe3 = dhe3, he3 = he3, dd = dd, dt = dt,
    converter = converter, second_converter = second_converter,
    collector = collector, tank = tank, tanks = tanks, heaters = heaters,
    pole = dhe3_pole,
  }
...
script.on_nth_tick(CHECK_AT, function()
  ...
  -- ------------------------------------------------------- the tank is a helium-3 vessel, only
  --
  -- REWRITTEN RATHER THAN DROPPED (#87). What stood here asserted that the tank buffered the tier's
  -- energy fluid, on a tank filled by insert_fluid and plumbed to nothing -- and Lua insertion
  -- ignores connection categories, so it would have passed unchanged after ADR 0018 item 5 took that
  -- role away. Both rows below go through plumbing, and the helium-3 one is the control that makes
  -- the refusal mean something.
  local he3_tank, energy_tank = r.tanks[1], r.tanks[2]
  record(#he3_tank.tank.fluidbox.get_connections(1) > 0,
    "the composite tank joins an ordinary helium-3 line, which is the role ADR 0018 leaves it")
  record(holds(he3_tank.tank, HELIUM_3) > 0, "and fills with helium-3 through it",
    string.format("%.6g units", holds(he3_tank.tank, HELIUM_3)))
  record(#energy_tank.tank.fluidbox.get_connections(1) == 0,
    "the same tank does NOT join a live aneutronic-energy line -- no vessel takes it through plumbing")
  record(holds(energy_tank.tank, ANEUTRONIC_ENERGY) == 0,
    "and holds not a unit of the tier's energy fluid",
    string.format("%.6g units", holds(energy_tank.tank, ANEUTRONIC_ENERGY)))
...
'@
    Set-Content -Encoding utf8 -Path (Join-Path $rigDir 'control.lua') `
        -Value $lua.Replace('__PLASMAFEED__', $feed).Replace('__ENERGYFEED__', $energyFeed).Replace('__TICKS__', "$Ticks")
}
