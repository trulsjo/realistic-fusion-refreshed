...
foreach ($n in $Connections) {
    foreach ($d in $Distances) {
        $cells += [pscustomobject]@{
            Id = "m-n$n-d$d"; Kind = 'matrix'; N = $n; D = $d; Volume = $Volume; Io = $false; Gap = 0
        }
    }
}
...
$cells += [pscustomobject]@{
    Id = 'c-nolink'; Kind = 'control'; N = $topN; D = $firstD; Volume = $Volume; Io = $false; Gap = 3
}
...
foreach ($d in $controlDistances) {
    $id = "c-cap2-d$d"
    $cells += [pscustomobject]@{
        Id = $id; Kind = 'control'; N = $topN; D = $d; Volume = $Volume * 2; Io = $false; Gap = 0
    }
    $controlOf[$id] = "m-n$topN-d$d"

    $cells += [pscustomobject]@{
        Id = "c-io-d$d"; Kind = 'control'; N = $topN; D = $d; Volume = $Volume; Io = $true; Gap = 0
    }
}
...
function Write-Rig {
    ...
    $specs = ($cells | ForEach-Object {
        '  {{ id = "{0}", n = {1}, d = {2}, volume = {3}, io = {4}, gap = {5} }},' -f
            $_.Id, $_.N, $_.D, $_.Volume, $_.Io.ToString().ToLowerInvariant(), $_.Gap
    }) -join "`n"
