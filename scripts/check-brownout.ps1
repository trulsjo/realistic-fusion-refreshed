...
function Write-BrownoutReport {
    ...
    # WHICH VERSION PRODUCED THE FIGURES (#109). Without it this file dates itself and nothing else,
    # and the figures move with the balance: the `dd` row read 70.58 MW when this note was first
    # rendered on 2026-08-20, 60.6 MW on 2026-08-23 and 85.23 MW on 2026-09-09, with nothing in the
    # file able to say what changed underneath it. The D-T rows are at least partly explicable --
    # the old render's own assertion table says "against a ceiling of 2e+09" where today's says 5e9,
    # so ADR 0025 moved those -- but the `dd` cell never reached either ceiling, and its move is
    # simply unattributed. A revision is what would have made it attributable, so it is recorded
    # from here on rather than reconstructed now.
    #
    # Tolerant of a repository with no git at all: an unknown revision is worth saying so about,
    # where a rig that refuses to write its report over it would be worse than the ambiguity.
    $revision = 'an unknown revision'
    try {
        $sha = (& git -C $repoRoot rev-parse --short HEAD 2>$null)
        if ($LASTEXITCODE -eq 0 -and $sha) {
            $dirty = (& git -C $repoRoot status --porcelain 2>$null)
            $revision = "``$sha``" + $(if ($dirty) { ' with uncommitted changes' } else { '' })
        }
    } catch { }
...
from this repository at **$revision**.
Verdict: **$verdict**
