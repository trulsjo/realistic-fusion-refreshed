...
function Get-QuietMapLua {
    <#  Lua defining rf_quiet_map(surface): stop the map fighting back for the length of a rig run.
        ...
        Shared rather than copied because check-brownout.ps1 is no longer the only rig long enough
        to be attacked -- probe-quality-equilibrium.ps1 runs twenty minutes and bench-mod-links.ps1
        thirty-five -- and three copies of a safety guard is where one starts to drift, silently.
...
local function rf_quiet_map(surface)
