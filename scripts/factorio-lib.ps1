...
function Get-RigBuildLua {
    <#  Lua defining the helpers every rig in this directory builds its map with (#226).
        ...
        safe. One implementation, so a guard added once is a guard every rig has.
...
--
-- The helpers every rig builds its map with. They were copied five ways until #226; the drift
...
if not rf_build_check then
  error("defines.build_check_type.manual is gone; every rig's placement guard would silently "
