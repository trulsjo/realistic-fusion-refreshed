...
.PARAMETER SelfTest
    ...
    from no fluid of ours, our barrel unlocks landing on a technology that carried none, on one
    whose only barrels are named for a fluid of ours -- the host list is built from the baseline
    dump, so a fluid of ours can never be what excuses a re-homing -- and on one whose only barrel
    unlock the game never generated, once per gate: a fluid of theirs carrying
    ...
    foreach ($key in $Replaced) {
        ...
        $foreign = switch ($SHAPES[$field]) {
            'unlock' {
                ...
                #   2. the destination technology ALREADY UNLOCKED barrels THE GAME GENERATED, in the
                #      baseline -- CONSTRUCTED the same way condition 1 constructs ours, as
                #      `<fluid>-barrel` and `empty-<fluid>-barrel` from the fluids present in the
                #      BASELINE dump, and gated the same way: the fluid does not set
                #      `auto_barrel = false`, and the `empty-` half of the pair is a recipe the
                #      baseline holds. That is what shows a pass of theirs was visibly already
                #      running before we arrived. A technology with no barrel unlock of any kind
                #      that gains only ours is not a re-homing and is still reported, and so is one
                #      whose only barrel unlock is a name the set wrote by hand.
                #
                #      IT WAS A CATEGORY TEST UNTIL #200, and the category is a label a set can swap
                #      wholesale: `angelspetrochem` recategorises every barrel recipe to
                #      `angels-barreling-pump`, so NOT ONE of `bob-fluid-barrel-processing`'s 144
                #      baseline unlocks named a `barrelling` recipe and the rule could never fire on
                #      an Angel's lane -- while the recipes were still base Factorio's, edited in
                #      place rather than generated by the set. The condition was always testing "a
                #      pass of theirs was visibly already running"; the category was only standing
                #      in for it. #194 chose the construction, ADR 0028 is the principle under it.
                #
                #      OUR FLUIDS CANNOT SATISFY THIS, BY CONSTRUCTION AND NOT BY EXCLUSION. The
                #      baseline is the game WITHOUT us, so no fluid of ours is in it and no barrel
                #      name of ours can be built from it. Do not add an explicit "not ours" clause:
                #      if one ever looks necessary, the list is being built from the wrong dump.
                #
                # ponytail: ONE baseline barrel unlock is enough, which is #192's threshold kept
                # deliberately by #194 -- moving mechanism and threshold in one change would make
                # any regression unattributable. It is the loosest joint in the rule: a technology
                # holding a single incidental barrel unlock excuses all 22 of ours landing on it.
                # Nothing observed on any lane wants a higher number -- measured 2026-09-02, Bob's
                # destination hosts on 30 of its 31 baseline unlocks and Angel's on 142 of 144 --
                ...
                # THEIRS, BUILT THE SAME WAY OURS WERE, off the baseline dump's own fluid entries.
                # Reading the recipe name apart instead -- stripping a `-barrel` suffix off each
                # baseline unlock -- would be the pattern match this file's rules exclude, and it
                # would count a hand-written `something-barrel` as the game's work.
                #
                # AND WITH THE SAME TWO GATES, WHICH IS WHAT "THE GAME MADE" MEANS. Constructing a
                # name is not evidence the engine generated it -- the same trap condition 1 fell
                # into for our six unbarrelled fluids, and it is no less a trap on this side. A set
                # is free to declare a fluid `auto_barrel = false`, hand-write a recipe under the
                # name the generation would have used, and unlock it; that technology would then
                # host, and all 22 of our unlocks landing on it would be excused as the set's doing
                # under a sentence saying base Factorio put them there. Caught in review of #200,
                # before it shipped.
                #
                # The two gates are the engine's own switch and the `empty-` half, which is the name
                # in the pair only the generation produces. What CANNOT be reused here is condition
                # 1's "present with us and absent without": that reads the appearance our fluid
                # caused, and their fluid predates both dumps. Presence in the baseline is the
                # strongest form of it available on this side.
                $theirBarrels = @{}
                foreach ($k in $Baseline.Keys) {
                    if (-not $k.StartsWith('fluid/', [StringComparison]::Ordinal)) { continue }
                    $f = $k.Substring('fluid/'.Length)
                    if ($false -eq ($Baseline[$k] | ConvertFrom-Json).auto_barrel) { continue }
                    if (-not $Baseline.ContainsKey("recipe/empty-$f-barrel"))      { continue }
                    $theirBarrels["$f-barrel"]       = $true
                    $theirBarrels["empty-$f-barrel"] = $true
                }
                $barrelHost = 0 -lt @($rowsBefore | Where-Object {
                    $_.type -eq 'unlock-recipe' -and $theirBarrels.ContainsKey($_.recipe)
                }).Count
