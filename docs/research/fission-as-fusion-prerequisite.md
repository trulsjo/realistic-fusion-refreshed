...
Read from `realistic-fusion-refreshed*/prototypes/technology/` on `main` at commit `e49d923`.
...
either module consumes, extracted from all nine recipe files in `realistic-fusion-refreshed/prototypes/recipes/`
and `realistic-fusion-refreshed-core/prototypes/recipes/`, is:
...
- ~~**`check_steam_sinks()` was read, not run.**~~ **Closed 2026-08-19.** It was an uncommitted change
  in `realistic-fusion-refreshed/control.lua` and `scripts/load-check.ps1` when this note was written, read as the
...
- `realistic-fusion-refreshed-core/prototypes/technology/{deuterium,lithium,mixing}.lua`;
  `realistic-fusion-refreshed/prototypes/technology/{d-d,d-t,blanket,aneutronic}.lua`.
