...
electric network, run for five minutes supplied, fifteen minutes short, and five minutes after. The
full measurement, with graphs, is regenerated by `-Report` into
[`brownout-rig.md`](brownout-rig.md); what it found:
...
- **Half supply is barely an event.** Cut to 27.4 MW against the 55.2 MW it was drawing, the cell
  stayed at 1.91×10⁹ °C and contributed **+96.1 MW net** across the shortfall.
- **A total blackout is a slow decline, and the reactor fights it the whole way.** Fifteen minutes
  with no power at all left the plasma at 6.74×10⁸ °C with 66.7 units still in the box, still
  selling **29.2%** of its lit output — and **+48.7 MW net**, since it is no longer paying for
  heating either. The same blackout took the D-D cell to **0.43%** and 6.6×10⁶ °C.
...
- **Recovery is unattended and, on this trajectory, free.** Supply restored and nothing else done,
  ...
  (+35.7 MW average). The cost that was expected here did not appear: the plasma never got thin
...
- **A row behaves like a reactor.** Two reactors bridged by `rf-pipe` on one plasma pool took the
  same blackout with a temperature spread of 0.0%: they share the fall rather than one starving the
  other.
