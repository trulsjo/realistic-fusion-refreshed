...
- **One class of silent mistake disappears.** `rf_assert_segments`, written in `bench-mod-links.ps1` and shared out of `Get-RigBuildLua` in `scripts/factorio-lib.ps1` since #226 by the five rigs that ticket consolidated, records it: an exchanger
